...
<div class="wrap">
    <div class="main">
        <div class="actual-wrap actual-w clearfix">
            ...
            <div class="map-box map-zhoushan fl">
            	<c:forEach var="pubScentDto" items="${pubScentDtoList}" varStatus="status">
            	<c:if test="${status.index  == 0}">
                <div class="city city-0"><i></i>舟山市本级<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 1}">
                <div class="city city-1">岱山县<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 2}">
                <div class="city city-2">定海区<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 3}">
                <div class="city city-3">洋山<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 4}">
                <div class="city city-4">金塘岛<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 5}">
                <div class="city city-5">新城<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 6}">
                <div class="city city-6">六横岛<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 7}">
                <div class="city city-7">嵊泗县<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 8}">
                <div class="city city-9">海洋产业集聚区<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 9}">
                <div class="city city-10">普陀山<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 10}">
                <div class="city city-11">普陀区<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                </c:forEach>
            </div>
