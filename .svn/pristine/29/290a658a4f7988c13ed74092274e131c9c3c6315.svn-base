require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate','jquery.multiselect'], function (layer, dataTable, util, http,handlebars) {
	...
	function initDataTable() {
		var url;
		if(state == 1)
			url = '/sment/rptsmBaseinfo/selectZxList.json';
		if(state == 2)
			url = '/sment/rptsmBaseinfo/selectSgsjBySetDeptCodeList.json';
		if(state == 3)
			url = "/sment/rptsmBaseinfo/selectJyxjBySetDeptCodeList.json";
		if(state == 4)
			url = "/sment/rptsmBaseinfo/selectJyycBySetDeptCodeList.json";
		if(state == 5)
			url = "/sment/rptsmBaseinfo/selectWfwgBySetDeptCodeList.json";
