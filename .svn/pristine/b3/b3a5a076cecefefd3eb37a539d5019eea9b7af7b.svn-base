    ...
    function bind() { 
        ...
        },
        {
            ...
            handler: function () { 
                layer.dialog({
                    title: '选择后置审批事项',
                    area: ['400px', '600px'],
                    content: _sysUrl+'/pubdtinfo/codeLicenseRadioTreePage',
        ...
        },
        {
            ...
            handler: function () { 
            	...
            	if(exaCode==null||exaCode==""){
            		layer.msg("请先选择后置审批事项", {ltype: 0,time:2000});
    	    		return false;
            	}
        ...
        },
        {
            ...
            handler: function () { 
            	...
            	$("#isValid").val(isValid);
            	$("#licDeptType").val(licDeptType);
            	$("#isSameGov").val(isSameGovTem);
            	//审批机关 
        	    ...
        	    if((liccode==""||liccode==null)||(licName==""||licName==null)){ 
    	    		layer.msg("请选择审批事项", {ltype: 0,time:2000});
    	    		return false;
    	    	} 
    	    	if((deptCode==""||deptCode==null)||(deptname==""||deptname==null)){ 
    	    		layer.msg("请选择对应审批部门", {ltype: 0,time:2000});
    	    		return false;
    	    	} 
    	    	...
    	    	if((orgNameText==null||orgNameText=="") ||(adCode==null||adCode=="")){
    	    		layer.msg("请选择实际审批部门", {ltype: 0,time:2000});
    	    		return false;
    	    	} 
        	    var formParam=$('#licDeptform').serializeObject();
    	    	if(_flag=="2"){//修改
   	                 http.httpRequest({
   	                    url: _sysUrl+'/licdept/commitLicDeptEdit',
   	                    serializable: true,
   	                    data: formParam,
   	                    type: 'post',
    	                    	...
    	                    	if (data.status == 'success') { 
    	                    		if(data.data=="1"){
    	                    			layer.confirm(data.msg, {icon: 3, title: '提示'}, function () { 
    	                 	                 http.httpRequest({ 
    	                 	                    url: _sysUrl+'/licdept/commitLicDeptUpdate', 
    	                 	                    serializable: true,
    	                  	                    data: formParam,
    	                  	                    type: 'post',
    	                 	                    success: function (data) {
    	    	   ...
    	    	   http.httpRequest({
  	                    url:_sysUrl+'/licdept/commitLicDeptAdd',
  	                    serializable: true,
  	                    data: formParam,
  	                    type: 'post',
   	                    success: function (data) {
