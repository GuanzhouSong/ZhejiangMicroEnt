require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate'], function (layer, dataTable, util, http, handlebars) {
  ...
  //合并第一列单元格
    ...
    function validForm(){
		...
		var auditState = $("#auditStateDiv input[name='auditState']:checked ").val();
		var auditOpin = $("#auditOpin").val();
		...
		if(remark == "" || remark.length > 200){
			...
			flag = false;
		}else if(!$("#scResult_1").prop("checked")){
			if(disposeScResult == ""){
		...
		if(flag){
			if(inspectDate == ""){
				...
				flag = false;
			}else if(auditState == "" || auditState == "undefined" || auditState == null){
				layer.tips("请选择审核结果",$("#auditStateDiv"),{tips:3,tipsMore:true,ltype:0});
				flag = false;
			}else if(auditOpin == ""){
				layer.tips("请输入审核意见",$("#auditOpin"),{tips:3,tipsMore:true,ltype:0});
				flag = false;
			}
		}
		if(!flag){
		...
		http.httpRequest({
			url: '/syn/pub/server/drcheck/pubscinfo/audit',
			dataType:"json",  
			...
			success: function (result) {
				if(result.data){
					...
					if(pubScresult != null && pubScresult != "" && pubScresult != "undefined"){
						...
						if(pubScresult.auditState != null && pubScresult.auditState == "5"){
							$("#auditStateDiv_other5").prop("checked",true);
						}else{
							$("#auditStateDiv_other5").prop("checked",false);
						}
    ...
    function getSaveParam(){
		 ...
		 $(".itemindex").each(function(){
			 ...
			 var pubScitemResult = new initPubScitemResult(itemid, itemCode, itemName, itemResult,resultTaskNO,priPID,scType,scName,sameFlag,pubFlag,auditState,auditOpin);
		 ...
		 var pubScresult = new initPubScresult($("#resultTaskNO").val(),priPID,scResult,$("#resultId").val(),$("#resultDetail").val(),remark,inspectDept,resultParamFlag,auditState,auditOpin,$("#checkDesc").val(),inspectDate);
    ...
    function initPubScitemResult(itemid,itemCode,itemName,itemResult,taskNO,priPID,scType,scName,sameFlag,pubFlag,auditState,auditOpin){
    	  this.id=itemid;
      ...
      function initPubScresult(taskNO,priPID,scResult,id,scDetail,remark,inspectDept,resultFlag,auditState,auditOpin,scDeptName,inspectDate){
      	this.taskNO=taskNO;
      	this.priPID=priPID;
