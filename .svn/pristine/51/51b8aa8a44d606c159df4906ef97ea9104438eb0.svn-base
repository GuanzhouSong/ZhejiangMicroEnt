...
function httpClick(supCode){
 http.httpRequest({
            url: '/reg/server/registinfo/registinfo/getSlicenoBynode',
            ...
            success: function (data) {
                ...
                if($('#sliceNO').val()==liData[i].code){
                check='checked';
                }else{check='';
                }
                 lis+='<div class="option-item"><label><input type="radio" name="codesliceno" '
                 +check+' value="'
    ...
    function bind() {
        ...
        {   el: '#save',
           ...
           layer.close({
           names: $('#sliceNOName').val(),
           codes: $('#sliceNO').val()
           });
        }
        },
    {
    el: 'input[name="codesliceno"]',
    event: 'click',
    handler: function () {
      var text = $(this).next().html();
      var val = $(this).val();
      $('#sliceNONamediv').html(text);
      $('#sliceNOName').val(text);
      $('#sliceNO').val(val);
    }
        },
            {
