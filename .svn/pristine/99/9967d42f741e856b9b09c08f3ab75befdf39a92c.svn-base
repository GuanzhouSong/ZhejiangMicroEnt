...
import com.icinfo.cs.dzdtinfo.mapper.LicDzDeptMapper;
...
/**
 * 描述:    cs_lic_dz_dept 对应的Service接口实现类.<br>
 *
 * @author framework generator
 * @date 2016年10月19日
 */
@Service
public class LicDzDeptServiceImpl extends MyBatisServiceSupport implements ILicDzDeptService {

	private static final Logger logger = LoggerFactory.getLogger(LicDzDeptServiceImpl.class);

	@Autowired
	private LicDzDeptMapper licDeptMapper;
	@Autowired
	 ...
	 */
	@Override
	public int insertLicDept(LicDzDept licDept) throws Exception {
		try {
			if(licDept!=null)
			{
	 ...
	 */
	@Override
	public LicDzDept selectLicDeptByExaCodeAndDeptCode(String exaCode,
		...
		try {
			...
			{
				LicDzDept licDept=new LicDzDept();
			...
			e.printStackTrace();
			logger.error("exception {}", "根据备案事项部门和部门编码和有效标识查询失败");
            throw new BusinessException("根据备案事项部门和部门编码和有效标识查询失败");
		}
	}
	 ...
	 */
	@Override
	public List<LicDzDept> selectLicDeptByExaCodeAndDeptCodeAndDutyDeptCode(String exaCode,
		...
		try {
			...
			e.printStackTrace();
			logger.error("exception {}", "根据备案事项部门和部门编码查询失败");
            throw new BusinessException("根据备案事项部门和部门编码失败");
		}
	}
	...
	public int deleteLicDeptByExaCodeAndDeptCode(String exaCode, String deptCode)
		...
		try {
			...
			e.printStackTrace();
			logger.error("exception {}", "根据备案事项编码和部门编码删除失败");
            throw new BusinessException("根据备案事项编码和部门编码删除失败");
		}
	}
	 ...
	 */
	@Override
	public List<LicDzDeptDto> queryLicDeptListJSON(PageRequest request)
			throws Exception { 
	...
	@Override
	public List<LicDzDeptDto> queryLicDeptByAdCode(String adCode)throws Exception{
	...
	/**
	 * 
	 * 描述   根据备案事项编码修改是否推送的值（licDeptType）
	 * @author 赵祥江
	...
	public int updateLicDeptByExaCode(String exaCode, String licDeptType,String flag) {
		try {
 			if(StringUtil.isNotEmpty(exaCode)&&StringUtil.isNotEmpty(licDeptType)){
				...
				Example example = new Example(LicDzDept.class);
	 ...
	 */
	@Override
	public int updateLicDeptByExaCodeAndDeptCode(LicDzDept licDept) {
		try {
			...
			e.printStackTrace();
			logger.error("exception {}", "根据备案事项编码和部门编码修改");
            throw new BusinessException("根据备案事项编码和部门编码修改");
		}
	}

	/**
	 * 
	 * 描述   根据备案部门级别自动生成个县市备案对照数据
	 * @author 赵祥江
	...
	public void executeLicDeptDataByLicZone() throws Exception { 
		try {
			logger.info("生成备案事项对照表数据开始");
			 ...
			 List<CodeDzLicense> codeLicenseList= codeLicenseService.selectCodeLicenseListByLicType("2");
			 int count =0;
			 String exaCodeStr="";
			 for(CodeDzLicense codeLicense:codeLicenseList){ 
						 ...
						 if("0".equals(licZoneArr[a])||"1".equals(licZoneArr[a])){
							 for(int b=0;b<licDeptTypeArr.length;b++){
								 ...
								 if(deptDutycodeRefListTem!=null&&deptDutycodeRefListTem.size()>0){
									 for(DeptDutycodeRef deptDutycodeRefTem:deptDutycodeRefListTem){
										 LicDzDept licDept=null; 
										//如果已经有了就不再新增
									     List<LicDzDept> LicDeptList=this.selectLicDeptByExaCodeAndDeptCodeAndDutyDeptCode(exaCode, deptDutycodeRefTem.getAdCode(),licDeptTypeArr[b]); 
							 ...
							 for(int c=0;c<licDeptTypeArr.length;c++){
								 ...
								 for(String cityLeveCode:cityLevelList){
	 								 ...
	 								 if(deptDutycodeRefListTem!=null&&deptDutycodeRefListTem.size()>0){
	 									 for(DeptDutycodeRef deptDutycodeRefTem:deptDutycodeRefListTem){
	 										LicDzDept licDept=getLicDept(deptDutycodeRefTem, exaCode); 
	 										//如果已经有了就不再新增
	 									     List<LicDzDept> LicDeptList=this.selectLicDeptByExaCodeAndDeptCodeAndDutyDeptCode(exaCode, deptDutycodeRefTem.getAdCode(),licDeptTypeArr[c]); 
	 										 if(LicDeptList==null||LicDeptList.size()==0){
	 											 licDept.setAdCode(licDept.getDeptCode());
												 licDept.setOrgName(licDept.getDeptName());
												 licDept.setYiedistrict(licDept.getDeptCode().substring(0, 6));
												 licDept.setIsSameGov("0");
							 ...
							 for(int d=0;d<licDeptTypeArr.length;d++){
								 ...
								 while(i.hasNext()){
									 ...
									 if(deptKey.indexOf(licDeptTypeArr[d])>0){
										 ...
										 LicDzDept licDept=getLicDept(deptDutycodeRefOwer, exaCode); 
										 List<LicDzDept> LicDeptList=this.selectLicDeptByExaCodeAndDeptCodeAndDutyDeptCode(exaCode, deptDutycodeRefOwer.getAdCode(),licDeptTypeArr[d]);
										 if(LicDeptList==null||LicDeptList.size()==0){ 
											 licDept.setAdCode(licDept.getDeptCode());
											 licDept.setOrgName(licDept.getDeptName());
											 licDept.setIsSameGov("0");
			  ...
			  if(exaCodeStr.length()>0){
				  logger.info("生成数据失败的备案事项编码为:"+exaCodeStr);
 			  }
			  logger.info("生成备案事项对照表数据结束"); 
 		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void  updateLicDeptData() throws Exception{
		List<LicDzDeptDto> licDeptDtoList=	this.queryLicDeptByAdCode(null);
		for(LicDzDeptDto licDeptDto: licDeptDtoList){
			LicDzDept licDept=new LicDzDept();
	 ...
	 * @throws
	 */
	public LicDzDept getLicDept(DeptDutycodeRef deptDutycodeRefTem,String exaCode){
		LicDzDept licDept=new LicDzDept();
		 licDept.setExaCode(exaCode);
		    	...
		    	for(SysUserCheckRecord sysUserCheckRecord:sysUserCheckRecordList){
		    		...
		    		if(StringUtil.isNotEmpty(sysUserCheckRecord.getLicItems())&&StringUtil.isNotEmpty(sysUserCheckRecord.getDutyCodes())){
			    		...
			    		for(int a=0;a<licItemsArr.length;a++){
			    			 ...
			    			 List<LicDzDept> LicDeptList=this.selectLicDeptByExaCodeAndDeptCodeAndDutyDeptCode(exaCode,adCode ,dutyCode);
							 if(LicDeptList==null||LicDeptList.size()==0){ 
	 ...
	 */
	@Override
	public List<LicDzDept> selectLicDeptByUid(String uid) throws Exception {
		LicDzDept licDept =new LicDzDept();
		if(StringUtil.isNotEmpty(uid)){
		   licDept.setUid(uid);
		   return licDeptMapper.select(licDept);
	 ...
	 */
	@Override
	public int updateLicDeptByUid(LicDzDept licDept) {
	 ...
	 */
	@Override
	public List<LicDzDept> selectLicDeptByExaCodeAndDutyDeptCodeAndYiedistrict(
	...
	public void transferExaCode(String exaCodeStr,String checkDep,String yFlag,String istransferFlag) throws Exception{
		try { 
			...
			CodeDzLicense codeLicense=codeLicenseService.selectCodeLicenseByExaCodeAndLicType(exaCodeStr, "2");
					...
					String licType=licTypeArr[i];
					 for(DeptTree deptTree: sysDepartList){
						 ...
						 if(deptTree.getAdcode().length()==6){
							 ...
							 List<LicDzDept>  licDeptList= this.selectLicDeptByExaCodeAndDutyDeptCodeAndYiedistrict(exaCodeStr, licType, adCode,"1");
							 if(licDeptList.size()>0){
								 for(LicDzDept licDept:licDeptList){
									 ...
									 if(deptDutycodeRefList.size()>0){
								    	 ...
								    	 if("1".equals(istransferFlag)){
								    		 List<PubDzDtInfoDto> pubDtInfoDtoList= pubDtInfoService.selectTransferPubdtinfoList(checkDep, exaCodeStr, "330000,330010");
									    	 if(pubDtInfoDtoList.size()>0){
									    		 for(PubDzDtInfoDto pubDtInfoDto:pubDtInfoDtoList){
									    			 ...
									    			 if(com.icinfo.cs.common.utils.StringUtil.isBlank(domDistrict)){
									    				 ...
									    				 List<LicDzDept>  licDeptListT= this.selectLicDeptByExaCodeAndDutyDeptCodeAndYiedistrict(exaCodeStr, licType, domDistrict,"1");
								    	 ...
								    	 List<PubDzDtInfoDto> pubDtInfoDtoList= pubDtInfoService.selectTransferPubdtinfoList(checkDep, exaCodeStr, adCode);
								    	 if(pubDtInfoDtoList.size()>0){
								    		 for(PubDzDtInfoDto pubDtInfoDto:pubDtInfoDtoList){
								    			 PubDzDtInfo pubDtInfo=new PubDzDtInfo();
								    			 pubDtInfo.setUid(pubDtInfoDto.getUid());
