require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate','pagination'], function (layer, dataTable, util, http,handlebars) {
	...
	//即时信息tab中的表初始化
	var table_jsgdyczxx,table_gqbgxx,table_jsxzxkxx,table_zscqczdjxx,table_jsxzcfxx;table_toushu
...
function initDataTable_pubOtherpunish(){ 
    	http.httpRequest({
            ...
            success: function(data) {
              ...
              handlebars.registerHelper("transDetail", function(data) { 
            	  var caseNO=data.caseNO; 
            	  var encryPriPID =window._CONFIG._encryPriPID;;
            	  var  text = '<a href="'+_urlHead+'getPunishdetail?caseNO='+caseNO+'&priPID='+encryPriPID+'" target="_blank">详情</a>';
                  return text;
			  }); 
...
//              {data: 'country',className: 'center'},
    ...
    //12315投诉申诉
    function initDataTable_Ai12315RegInfo(){
    	table_toushu = dataTable.load({
    		el:'#table_toushu',
    		showIndex:true,
    		lengthMenu: [ 5, 10, 20, 50],
    		ajax:{
    			url:'/syn/panoramasearch/selectAi12315RegInfoJSON',
    			data :function(d){
    				d.params = params;
    			}
    		},
    		columns : [
    			{data: null, className: 'center'},
    			{data: 'infoType', className: 'center'},
    			{data: 'regTime', className: 'center'},
    			{data: 'applBasque', className: 'center'}, 
    			{data: 'brist', className: 'center'}
    	],			
    		columnDefs : [
    		    {
    		    	targets:1,
  		       ...
  		       },
	  		     {
	   		    	targets: 3,
					      render: function (data, type, row, meta) {
					    	    var infoType=row.infoType;
					    	    if(infoType=="1"){
					    	    	   //申诉基本问题 
					    	    	   var applidique=row.applidique==null?"":row.applidique;
					    	    	   if(applidique!=""&&applidique.length>20){
				 							return "<span title='"+applidique+"'>"+applidique.substring(0,20)+"...</span>";
				 					   }else{
				 							return applidique;
				 					   }
					    	    }else if(infoType=="2"){
					    	    	   //举报基本问题 
					    	    	   var reportidique=row.reportidique==null?"":row.reportidique;
					    	    	   if(reportidique!=""&&reportidique.length>20){
				 							return "<span title='"+reportidique+"'>"+reportidique.substring(0,20)+"...</span>";
				 					   }else{
				 							return reportidique;
				 					   }
					    	    }else {
    		...
    		"fnDrawCallback": function (oSettings) {
      			 $("#tab_toushuTotal").text(oSettings._iRecordsTotal);
      			 if(oSettings._iRecordsTotal==0 && initFlag){
      				initAddflex("tab_toushu_li","table_toushu");
      			 }
      		}
    	});
    	table_toushu.on('xhr.dt',function(e,settings,json,xhr){
    		if(json.recordsTotal <= 5){$('#table_toushu_wrapper .bottom').hide();}else{$('#table_toushu_wrapper .bottom').show();}
        })
    }
    ...
    function bind(){
        ...
        {
            el: '#tab_toushu_li',//投诉申诉
            event: 'click',
            handler: function () {
            	addflex("tab_toushu_li","table_toushu");
            }
        }, 
        {
            el: '#entzlinfo',
    ...
    function getTabTotal() { 
        http.httpRequest({
            ...
            success: function (data) {
                ...
                //12315投诉申诉
                var toushuCount = data.data.toushuCount == null ? 0 : data.data.toushuCount;
                //良好荣誉
                ...
                $("#toushuCount").text("("+toushuCount+")");
                if(toushuCount>0){
               	 $("#toushuCount").css("color","red");
                } 
