require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate'], function (layer, dataTable, util, http,handlebars) {
    ...
    function initDataTable() {
        table = dataTable.load({
			...
			scrollX:true, //支持滚动
            ajax: {
                url:'/pub/server/pubotherpunishinput/panoQueryPage',
                data:function(d){
    ...
    function bind() {
        util.bindEvents([{
            ...
            handler: function () {
                  ...
                  var typeUrl="";
                  if(userType=="2"){
                	  window.location.href="/syn/server/pubotherpunishinput/list";
                  }else{
                	  window.location.href="/pub/server/pubotherpunishinput/list";  
                  }
              }
        },{
            el: '#addBtn',
            ...
            handler: function () {
            	...
            	if(checkFlag){
	                layer.dialog({
	                    title: '添加',
	                    area: ['70%', '60%'],
	                    content: typeUrl+'/server/pubotherpunishinput/addOrEditView?pripid='+data.priPID+'&entName='+data.entName+'&regNO='+data.regNO+'&leRep='+data.leRep,
	                    callback: function (data) {
	                        if (data.reload) {
	                        	window.location.href=typeUrl+"/server/pubotherpunishinput/list";
	                        }
	                    }
	                })
                }else{
