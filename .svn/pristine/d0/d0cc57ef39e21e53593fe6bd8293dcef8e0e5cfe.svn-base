...
/*    $.validator.addMethod("placeholder", function(value, element) {
    ...
    $.validator.addMethod("isTel", function(value, element) {
        var reg =  /^\d{4}-?\d{7,8}$|^\d{7,8}$|^\d{3}-?\d{3}-?\d{4}$/;
        return this.optional(element) || (reg.test(value));
    }, "请正确填写电话号码");
     ...
     * 电话号码/手机号验证规则
     */
    $.validator.addMethod("telandphone", function(value, element) {
        var length = value.length;
    ...
    $.validator.addMethod("checkMoney", function(value, element) {
        var flag = false;
        if(parseFloat(value)>0){flag = true;}
     ...
     * 百分比验证规则 保留小数点后面2位
     */
    $.validator.addMethod("percentTwo", function(value, element) {
        var reg = /^(100(\.0{1,2})?|[1-9]\d(\.\d{1,2})?|\d(\.\d{1,2})?)$/;
        return this.optional(element) || (reg.test(value));
    }, "请正确填写百分比,最多保留小数点后两位");
