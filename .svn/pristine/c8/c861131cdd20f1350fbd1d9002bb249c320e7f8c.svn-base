require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate'], function (layer, dataTable, util, http,handlebars) {
    ...
    function initDataTable() {
        table = dataTable.load({
			...
			scrollX:true, //支持滚动
            ajax: {
                url:window._CONFIG.url + '/favorableinput/panoQueryPage',
                data:function(d){
    ...
    function bind() {
        util.bindEvents([{
            ...
            event: 'click',
            handler: function () {
            	 window.location.href = window._CONFIG.url + "/favorableinput/list";
              }
        },{
            el: '#addBtn',
            event: 'click',
            handler: function () {
            	...
            	if(checkFlag){
	                layer.dialog({
	                    title: '添加',
	                    area: ['65%', '72%'],
	                    content: window._CONFIG.url+'/favorableinput/addOrEditView?priPID='+data.priPID,
