<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<!doctype html>
<html lang="en">
<head>
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="renderer" content="webkit">
    <meta charset="utf-8">
    <title>详情_工商登记信息</title>
    <link rel="stylesheet" href="/css/syn.css">
</head>
<body>
<div class=" clearfix pd20">
    <h2 class="center hd-underline border-bottom">涉证企业操作详情</h2>
    <div class="tabbale" data-component="tab">
        <div class="tab-header">
            <ul class="clearfix type-tab clear">
                <li class="tab-selected"><span class="tab-item"><a href="javascript:void(0);">工商登记信息</a></span></li>
                <li><span class="tab-item"><a href="javascript:void(0);">许可审批信息</a></span></li>
                <li><span class="tab-item"><a href="javascript:void(0);">日常监管信息</a></span></li>
                <li><span class="tab-item"><a href="javascript:void(0);">预警警示信息</a></span></li>
            </ul>
        </div>
<!--         <div class="tab-content"> -->
            <div class="tab-panel">
                <div class="">
                    <h4 class="h4-title mt10">企业信息</h4>
                    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="table-horizontal">
                        <tbody>
                        <tr>
                            <td class="bg-gray right" width="10%">统一代码</td>
                            <td width="40%"></td>
                            <td class="bg-gray right" width="10%">注册号</td>
                            <td>330182600202310</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">企业名称</td>
                            <td>龙泉市八百里龙泉人家农家乐餐馆</td>
                            <td class="bg-gray right">注册资本</td>
                            <td>10d万元</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">是否个转企</td>
                            <td>否</td>
                            <td class="bg-gray right">住所</td>
                            <td>浙江省龙泉</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">企业类型</td>
                            <td>个人独资企业</td>
                            <td class="bg-gray right">行业类型</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">成立日期</td>
                            <td>2013年11月12日</td>
                            <td class="bg-gray right">核准日期</td>
                            <td>2013年11月12日</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">经营范围</td>
                            <td colspan="3">餐饮服务：农家乐，不含凉菜（餐饮服务许可证有效期至2014年09月20日）；会议服务；卷烟、雪茄烟零售（凭有效的烟草专卖零售许可证经营）。</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">推送方式</td>
                            <td>自动推送</td>
                            <td class="bg-gray right">推送日期</td>
                            <td>2013年11月12日</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">审批机关</td>
                            <td></td>
                            <td class="bg-gray right light">后置审批事项名称</td>
                            <td class="light">乙级、丙级职业卫生技术服务机构资质认可</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">法定代表人</td>
                            <td>叶</td>
                            <td class="bg-gray right">联系电话</td>
                            <td>15815457554</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">企业联络员</td>
                            <td>叶</td>
                            <td class="bg-gray right">联系电话</td>
                            <td>15815457554</td>
                        </tr>
                        <tr>
                            <td class="bg-gray right">登录机关联络员</td>
                            <td>周某某</td>
                            <td class="bg-gray right">联系电话</td>
                            <td>15815457554</td>
                        </tr>
                        </tbody>
                    </table>
                    <h4 class="h4-title mt10">接收信息</h4>
                    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="table-horizontal">
                        <tr>
                            <td class="bg-gray right" width="15%">统一代码</td>
                            <td>建德市洋溪街道徐盼副食品商店</td>
                            <td class="bg-gray right" width="15%">企业统一代码/注册号</td>
                            <td>330182600202310</td>
                        </tr>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
<%--<p class="type-tab pdb10"><a href="javascript:void(0);" class="cur">工商登记信息</a><a href="javascript:void(0);">许可审批信息</a><a href="javascript:void(0);">日常监管信息</a><a href="javascript:void(0);">预警警示信息</a></p>--%>


</div>
</body>
</html>