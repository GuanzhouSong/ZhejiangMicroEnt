            ...
            columnDefs: [
                         {
                             ...
                             render: function (data, type, row, meta) { 
                             	  if(row.auditState=='0'){
                                      return '<a href="javascript:void(0);" class="js-edit" id='+row.uid+' name='+row.entName+'>审核</a>';
                                     }
                                    else{
            ...
            handler: function () {
            	var uid=this.id;
             	layer.dialog({
                    title: '审核',
                    area: ['80%', '73%'],
                    content: '/reg/server/orderreformset/orderReformSetAuditEditPage?uid='+uid+'&viewFlag=1',
                    callback: function (data) {
                        if (data.reload) {
                            table.ajax.reload();
                        }
