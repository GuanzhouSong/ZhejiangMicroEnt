...
public class SysUserRoleServiceImpl extends MyBatisServiceSupport implements ISysUserRoleService {
     ...
     */
    @Override
    public int deleteBySysUserRole(SysUserRole sysUserRole) throws Exception {
        if(StringUtil.isNotBlank(sysUserRole.getRoleId())&&StringUtil.isNotBlank(sysUserRole.getUserId())){
            return sysUserRoleMapper.delete(sysUserRole);
        }else{
            throw new BusinessException("删除用户角色失败,角色ID或者用户ID为空~");
        }
    }
