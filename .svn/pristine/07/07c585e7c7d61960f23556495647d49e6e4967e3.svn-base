require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','pagination'], function (layer, dataTable, util, http, handlebars) {
    ...
    function init() {
    	...
    	if(entTypeCatg==16||entTypeCatg==17){
            initDataTable_sfc_licenceinfo();//农专年报行政许可
            initDataTable_sfc_websiteinfo();//农专网站或网店信息
            initDataTable_sfc_branchinfo();//农专分支机构信息
            initDataTable_sfc_updateinfo();//农专年报修改
    	}else if(entTypeCatg==50){
            initDataTable_pb_licenceinfo();//个体户年报行政许可
            initDataTable_pb_websiteinfo();//个体户网站或网店信息
            initDataTable_pb_updateinfo();//个体户年报修改
    	}else{
    		var repType = window._CONFIG.repType;
    		initDataTable_website();//网站或网店信息
    		
    		if (repType.indexOf("subcapital") != -1) {
    			 initDataTable_inv();//股东及出资信息
    		}
    		if (repType.indexOf("forinvestment") != -1) {
    			initDataTable_forinvestment();//对外投资信息
    		}
    		initDataTable_forguaranteeinfo();//对外提供保证担保信息
    		if (repType.indexOf("alterstockinfo") != -1) {
    			initDataTable_alterstockinfo();//股权变更信息
    		}
            initDataTable_updateinfo();//修改信息
    	}
    ...
    //网站或网店信息
    function initDataTable_website(){
    	...
    	http.httpRequest({
            ...
            success: function (data) {
            	$('#website').text("共计"+data.length+"条信息");
            	$('#websitelight').text("共计"+data.length+"条信息");
            	var jointHtml="";
            	...
            	$(".website").append(jointHtml);
            }
        });
    }

    //股东及出资信息
    function initDataTable_inv(){
        loadData_inv(0,5);
    }
    function loadData_inv(page, length) {
        var params = $('#params').serializeObject();//查询参数
        ...
        http.httpRequest({
            ...
            success: function (data) {
                ...
                $('#pagination_inv').pagination(itemCount, {
...
//					}
                ...
                var myTemplate = handlebars.compile($('#table-template-inv').html());
                if(typeof itemCount!='undefined' &&itemCount>0) $('#table-inv').html(myTemplate(data));
                $('#_total_inv').text(itemCount);//总数
                var pageNum = itemCount%5==0?itemCount/5:Math.ceil(itemCount/5);
                $('#_pageNum_inv').text(pageNum);//共多少页
            }
        });
    };

    //对外投资信息
    function initDataTable_forinvestment(){
        var params = $('#params').serializeObject();//查询参数
    	http.httpRequest({
            ...
            type: 'post',
            success: function (data) {
            	$('#yrforinvestment').text("共计"+data.length+"条信息");
            	var jointHtml="";
            	for(var i=0;i<data.length;i++){
            	...
            	$(".forinvestment").append(jointHtml);
            }
        });
    }
    ...
    function initDataTable_forguaranteeinfo(){
        loadData_forguaranteeinfo(0,5);
    };
    function loadData_forguaranteeinfo(page, length) {
        params = $('#params').serializeObject();//查询参数
        http.httpRequest({
            ...
            success: function (data) {
                var itemCount = data.recordsTotal; //获取查询出来的总数
                $('#pagination_forguaranteeinfo').pagination(itemCount, {
                    prev_text: '<i class="icon-triangle-left"></i>上一页',
                    next_text: '下一页<i class="icon-triangle-right"></i>',
                    num_display_entries: 3,//主体页数
                    num_edge_entries: 2,//边缘页数
                    ...
                    last_text: '末页',
                    callback:function(page,jq){
                    	loadData_forguaranteeinfo(page,5);
                    }
                });
                var handleHelper = handlebars.registerHelper("xh", function(index) {return index+1+(page*5);});//序号
...
//					}
                ...
                var myTemplate = handlebars.compile($('#table-template-forguaranteeinfo').html());
                if(typeof itemCount!='undefined' &&itemCount>0) $('#table-forguaranteeinfo').html(myTemplate(data));
                $('#_total_forguaranteeinfo').text(itemCount);//总数
                var pageNum = itemCount%5==0?itemCount/5:Math.ceil(itemCount/5);
                $('#_pageNum_forguaranteeinfo').text(pageNum);//共多少页
            }
        });
    };

    //股权变更信息
    function initDataTable_alterstockinfo(){
        loadData_alterstockinfo(0,5);
    };
    function loadData_alterstockinfo(page, length) {
        var params = $('#params').serializeObject();//查询参数
        http.httpRequest({
            ...
            success: function (data) {
                var itemCount = data.recordsTotal; //获取查询出来的总数
                $('#pagination_alterstockinfo').pagination(itemCount, {
                    prev_text: '<i class="icon-triangle-left"></i>上一页',
                    next_text: '下一页<i class="icon-triangle-right"></i>',
                    num_display_entries: 3,//主体页数
                    num_edge_entries: 2,//边缘页数
                    ...
                    last_text: '末页',
                    callback:function(page,jq){
                    	loadData_alterstockinfo(page,5);
                    }
                });
                var handleHelper = handlebars.registerHelper("xh", function(index) {return index+1+(page*5);});//序号
...
// 					}
                ...
                var myTemplate = handlebars.compile($('#table-template-alterstockinfo').html());
                if(typeof itemCount!='undefined' &&itemCount>0) $('#table-alterstockinfo').html(myTemplate(data));
                $('#_total_alterstockinfo').text(itemCount);//总数
                var pageNum = itemCount%5==0?itemCount/5:Math.ceil(itemCount/5);
                $('#_pageNum_alterstockinfo').text(pageNum);//共多少页
            }
        });
    };
                ...
                handlebars.registerHelper("altBe", function(data) {
                	if(data.altBe =="undefined" || data.altBe == null ){
 					...
 					if (data.altBe.indexOf("CST") > -1 ||data.altBe.indexOf("GMT") > -1) {
 					    ...
 					    d = d < 10 ? ('0' + d) : d;
 						return y + '年' + m + '月' + d+"日";
 					} else if (data.altItem.toUpperCase().indexOf("ISPUB") > -1) {
 						if(data.altBe==1){
 							return '是';
 						}else{
 							return "否";
 						}
 					} else if (data.altItem.toUpperCase().indexOf("WEBTYPE") > -1) {
 						if(data.altBe==1){
 							return '网站';
 						}else{
 							return "网店";
    ...
    //个体户行政许可
    function initDataTable_pb_licenceinfo(){
        loadData_pb_licenceinfo(0,5);
    };
    function loadData_pb_licenceinfo(page, length) {
        var params = $('#params').serializeObject();//查询参数
        http.httpRequest({
            ...
            success: function (data) {
                var itemCount = data.recordsTotal; //获取查询出来的总数
                $('#pagination_pb_licenceinfo').pagination(itemCount, {
                    prev_text: '<i class="icon-triangle-left"></i>上一页',
                    next_text: '下一页<i class="icon-triangle-right"></i>',
                    num_display_entries: 3,//主体页数
                    num_edge_entries: 2,//边缘页数
                    ...
                    last_text: '末页',
                    callback:function(page,jq){
                    	loadData_pb_licenceinfo(page,5);
                    }
                });
                var handleHelper = handlebars.registerHelper("xh", function(index) {return index+1+(page*5);});//序号
...
//					}
                ...
                var myTemplate = handlebars.compile($('#table-template-pb_licenceinfo').html());
                if(typeof itemCount!='undefined' &&itemCount>0) $('#table-pb_licenceinfo').html(myTemplate(data));
                $('#_total_pb_licenceinfo').text(itemCount);//总数
                var pageNum = itemCount%5==0?itemCount/5:Math.ceil(itemCount/5);
                $('#_pageNum_pb_licenceinfo').text(pageNum);//共多少页
            }
        });
    };

    //个体户网站信息
    function initDataTable_pb_websiteinfo(){
        ...
        http.httpRequest({
            ...
            type: 'post',
            success: function (data) {
            	$('#pbwebsite').text("共计"+data.length+"条信息");
            	$('#pbwebsitelight').text("共计"+data.length+"条信息");
            	var jointHtml="";
            	for(var i=0;i<data.length;i++){
            		var webSite = data[i].webSite;//网址
            		var webType = data[i].webType;//网站类型
    ...
    //农专行政许可
    function initDataTable_sfc_licenceinfo(){
        loadData_sfc_licenceinfo(0,5);
    }
    function loadData_sfc_licenceinfo(page, length) {
        var params = $('#params').serializeObject();//查询参数
        http.httpRequest({
            ...
            success: function (data) {
                var itemCount = data.recordsTotal; //获取查询出来的总数
                $('#pagination_sfc_licenceinfo').pagination(itemCount, {
                    prev_text: '<i class="icon-triangle-left"></i>上一页',
                    next_text: '下一页<i class="icon-triangle-right"></i>',
                    num_display_entries: 3,//主体页数
                    num_edge_entries: 2,//边缘页数
                    ...
                    last_text: '末页',
                    callback:function(page,jq){
                    	loadData_sfc_licenceinfo(page,5);
                    }
                });
                var handleHelper = handlebars.registerHelper("xh", function(index) {return index+1+(page*5);});//序号
                handlebars.registerHelper("licNameCN", function(data) {
...
//					}
                ...
                var myTemplate = handlebars.compile($('#table-template-sfc_licenceinfo').html());
                if(typeof itemCount!='undefined' &&itemCount>0)  $('#table-sfc_licenceinfo').html(myTemplate(data));
                $('#_total_sfc_licenceinfo').text(itemCount);//总数
                var pageNum = itemCount%5==0?itemCount/5:Math.ceil(itemCount/5);
                $('#_pageNum_sfc_licenceinfo').text(pageNum);//共多少页
            }
        });
    };

    //农专网站或网店信息
    function initDataTable_sfc_websiteinfo(){
        var params = $('#params').serializeObject();//查询参数
        http.httpRequest({
            ...
            type: 'post',
            success: function (data) {
            	$('#sfcwebsite').text("共计"+data.length+"条信息");
            	$('#sfcwebsitelight').text("共计"+data.length+"条信息");
            	var jointHtml="";
            	for(var i=0;i<data.length;i++){
            		var webSite = data[i].webSite;//网址
            		var webType = data[i].webType;//网站类型
            	...
            	$(".sfc_website").append(jointHtml);
            }
        });
    }

    //农专分支机构信息
    function initDataTable_sfc_branchinfo(){
        loadData_sfc_branchinfo(0,5);
    }
    function loadData_sfc_branchinfo(page, length) {
        var params = $('#params').serializeObject();//查询参数
        http.httpRequest({
            ...
            success: function (data) {
                var itemCount = data.recordsTotal; //获取查询出来的总数
                $('#pagination_sfc_branchinfo').pagination(itemCount, {
                    prev_text: '<i class="icon-triangle-left"></i>上一页',
                    next_text: '下一页<i class="icon-triangle-right"></i>',
                    num_display_entries: 3,//主体页数
                    num_edge_entries: 2,//边缘页数
                    ...
                    last_text: '末页',
                    callback:function(page,jq){
                    	loadData_sfc_branchinfo(page,5);
                    }
                });
                var handleHelper = handlebars.registerHelper("xh", function(index) {return index+1+(page*5);});//序号
...
//					}
                ...
                var myTemplate = handlebars.compile($('#table-template-sfc_branchinfo').html());
                if(typeof itemCount!='undefined' &&itemCount>0) $('#table-sfc_branchinfo').html(myTemplate(data));
                $('#_total_sfc_branchinfo').text(itemCount);//总数
                var pageNum = itemCount%5==0?itemCount/5:Math.ceil(itemCount/5);
                $('#_pageNum_sfc_branchinfo').text(pageNum);//共多少页
            }
        });
    };
