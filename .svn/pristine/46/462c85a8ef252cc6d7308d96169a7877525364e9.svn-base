...
<div>
    <table border="0" cellspacing="0" cellpadding="0" class="table-row perc-100 mt30">
        ...
        <tbody>
             <c:forEach var="csEntWarn"   items="${csEntWarnList}"   varStatus="status">
                 <tr>
                    <td >${status.index+1}</td>
                    <td>${csEntWarn.markName}</td>
                 </tr>
             </c:forEach>
        </tbody>
    </table>
...
<script src="/js/syn/system/risk/entwarninfo/entwarnapply_warnhis.js"></script>
</body>
</html>