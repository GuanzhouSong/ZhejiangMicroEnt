...
function(layer, dataTable, util, http, handlebars) {
    ...
    function initDataTable(page,length,callback) { 
    	var params={"year":window._CONFIG._year,
    		...
    		"pageNum":page};
        http.httpRequest({
            url: '/reg/client/yr/sfc/yrprint/list.json',
            serializable: false,
            showIndex: true, 
   ...
   function bind() {
       util.bindEvents([ 
       {
           el: '.printSfcHisReport',
           event: 'click',
           handler: function () { 
               var _id = "_doPrint_"+new Date().getTime();
                var url = "/reg/client/yr/sfc/yrprint/readSfcHisReportPage?reportNo="+this.id+"&anCheID="+this.title+"&printFalg=2";
               try{
       ...
       },
       {
           ...
           handler: function () { 
               var _id = "_doPrint_"+new Date().getTime();
               var url = "/reg/client/yr/sfc/yrprint/readSfcHisReportPage?reportNo="+this.id+"&anCheID="+this.title+"&printFalg=1";
               /*try{
                   window.open(url, _id, "menubar=no,location=no,resizable=no,scrollbars=yes,status=no");
               }catch(e){
                   alert("打印错误!");
