require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate'], function (layer, dataTable, util, http,handlebars) {
    ...
    function bind() {
        util.bindEvents([{
            el: '#addMove',//进入移出申请添加list页面
            event: 'click',
            handler: function () {
            	window.location.href='/reg/server/opadetail/pubopadetail/opadetailAddMoveList?moveFlag=1&systemType='+window._CONFIG._systemType;
            }
        },{
            el: '.view',
            event: 'click',
            ...
            handler: function () {
            	...
            	layer.dialog({
                    title: '经营异常名录移出申请 - 修改',
                    area: ['80%', '75%'],
                    content: '/reg/server/opadetail/pubopadetail/opadetailAddMove?busExcList='+busExcList+"&operate=1",
                    callback: function (data) {
                        if (data.reload) {
                            table.ajax.reload();
                        }
