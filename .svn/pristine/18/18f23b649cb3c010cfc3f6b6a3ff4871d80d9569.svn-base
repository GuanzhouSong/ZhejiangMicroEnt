require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate','pagination','ztree','ztreeCheck'], function (layer, dataTable, util, http,handlebars) {
    ...
    function initView(idname){
            ...
            data:{idname:idname,priPID:$("#priPID").val(),isShowCheckOnly:true},
            type: 'post',
            success: function(data) {
            	if (data && data.status == 'success') { 
                  //初始化权限树
    ...
    function bind(){
        ...
        },
        {
    		...
    		handler: function () {
    			var markCodes = "";
    			var markNames = "";
    			$('#js-select-add option').each(function (index){
    				var val = $(this).val();
    				var text = $(this).attr("data-text");
    				markCodes += (index >0?",":"")+val;
    				markNames += (index >0?",":"")+text;
    		...
    		handler: function () {
    			...
    			var auditorDescTem = $.trim($("#auditorDesc").val());
