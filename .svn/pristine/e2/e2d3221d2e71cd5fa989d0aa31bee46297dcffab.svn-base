require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate'], function (layer, dataTable, util, http,handlebars) {
    ...
    function initDataTable() { 
        table = dataTable.load({
            ...
            showIndex: true,
            ajax: {
                url:window._CONFIG.chooseUrl+'/pubdzdtinfo/searchlist.json',
                data:function(d){
                	if(clickFlag){
...
//		 			            	console.log(now)
							...
							},
							{
								targets : 3 ,
								render : function(data, type, row, meta) {
									if(row.checkDeptType != '1' && row.receiveState == '0'){
        ...
        util.bindEvents([{//查看
              ...
              handler: function () {
                  ...
                  layer.dialog({
                      title: '',
                      area: ['65%', '68%'],
                      content: window._CONFIG.chooseUrl+'/pubdzdtinfo/searchDetailInfo?priPId='+data.priPID+'&uid='+data.uid,
          ...
          handler: function () {
            var data = table.row($(this).closest('td')).data();
              layer.dialog({
                  title: '',
                  area: ['90%', '90%'],
                  content: window._CONFIG.chooseUrl+'/pubdzdtinfo/receive?priPId='+data.priPID+'&uid='+data.uid,
                  callback: function (data) {
                	//重新加载列表数据
                      if (data.reload) {
                    	  table.ajax.reload();
    ...
    },
    {
        ...
        handler: function () {
           ...
           function(index) {
        	   http.httpRequest({
                   url: window._CONFIG.chooseUrl+'/pubdzdtinfo/doreceiveRtn',
                   data: {
                   	uids:ids.toString()
                   },
                   type: 'post',
  ...
  },
  {
      ...
      handler: function () { 
          layer.dialog({
              title: '选择备案事项',
              area: ['500px', '600px'],
              content: window._CONFIG.chooseUrl+'/pubdzdtinfo/codeLicenseCheckboxTreePage',
              callback: function (data) { 
