    			...
    			columnDefs: [
    				{
    					...
    					render: function (data, type, row, meta) {
    						...
    						if("1"==auditState){
    							return '<a href="javascript:void(0);" id="'+row.markUuid+'" data-priPID="'+row.priPID+'" class="audit">审核</a>';
    						}else{
    	...
    	util.bindEvents([{
    		...
    		handler: function () {
    			...
    			layer.dialog({
    				title: '分类标签标注-审核',
    				area: ['70%', '95%'],
    				content: '/reg/server/mainmark/toMainmarkauditEdit?markUuid='+id+'&priPID='+priPID,
    				callback: function (data) {
	                   	 if (data.reload) { //close是否有传递此参数
	                         table.ajax.reload();
	                     }
