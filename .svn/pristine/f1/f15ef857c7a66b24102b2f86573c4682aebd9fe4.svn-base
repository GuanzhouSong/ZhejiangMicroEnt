...
public class PubDtInfoServiceImpl extends MyBatisServiceSupport implements IPubDtInfoService {
	...
	public PubDtInfoDto doGetDtCommonDataForCharts(PageRequest request) throws Exception{
		PubDtInfoDto infoDto = new PubDtInfoDto();
		Map<String, Object> qryMap = request.getParams();
		String checkDeptAmout = pubDtInfoMapper.selectCheckDepTotalForCharts(qryMap);
		String pushAmount = pubDtInfoMapper.selectPushTotalForCharts(qryMap);
		String claimAmout = pubDtInfoMapper.selectClaimTotalForCharts(qryMap);
		infoDto.setCheckDeptAmount(checkDeptAmout);
		infoDto.setPushAmount(pushAmount);
		infoDto.setClaimAmount(claimAmout);
		return infoDto;
	}
	...
	public PubDtInfoDto doGetDutyClaimOptForCharts(PageRequest request) throws Exception{
		PubDtInfoDto infoDto = new PubDtInfoDto();
		String claimAmout = pubDtInfoMapper.selectClaimTotalForCharts(request.getParams());
		String backCount = pubDtInfoMapper.selectBackTotalForCharts(request.getParams());
		String noClaimCount = pubDtInfoMapper.selectNoClaimCountForCharts(request.getParams());
		int hasHandleCount = Integer.valueOf(claimAmout) + Integer.valueOf(backCount);
		infoDto.setClaimAmount(claimAmout);
		infoDto.setBackCount(backCount);
		infoDto.setNoClaimCount(noClaimCount);
		infoDto.setHasHandleCount(String.valueOf(hasHandleCount));
		return infoDto;
	}
	...
	public PubDtInfoDto doGetDeptClaimDataForCharts(PageRequest request) throws Exception{
		PubDtInfoDto infoDto = new PubDtInfoDto();
		Map<String, Object> qryMap = request.getParams();
		String areaCode = qryMap.get("areaCode").toString();
		if (!"3300".equals(areaCode) && StringUtil.isNotEmpty(areaCode)) {
			//市级部门认领个数
			String cityClaimAmount = pubDtInfoMapper.selectClaimTotalForCity(qryMap);
			//县级部门认领个数
			String countyClaimAmount = pubDtInfoMapper.selectClaimTotalForCounty(qryMap);
			infoDto.setProClaimAmount("0");
			infoDto.setCityClaimAmount(cityClaimAmount);
			infoDto.setCountyClaimAmount(countyClaimAmount);
		}else if("3300".equals(areaCode)){
			//省级部门认领个数
			String proClaimAmount = pubDtInfoMapper.selectClaimTotalForPro(qryMap);
			infoDto.setProClaimAmount(proClaimAmount);
			infoDto.setCityClaimAmount("0");
			infoDto.setCountyClaimAmount("0");
		}else{
			String proClaimAmount = pubDtInfoMapper.selectClaimTotalForPro(qryMap);
			String cityClaimAmount = pubDtInfoMapper.selectClaimTotalForCity(qryMap);
			String countyClaimAmount = pubDtInfoMapper.selectClaimTotalForCounty(qryMap);
	...
	public List<PubDtInfoDto> doGetClaimInfoForDutyData(PageRequest request) throws Exception{
		return pubDtInfoMapper.selectClaimInfoForDuty(request.getParams());
	}
	...
	public  List<PubDtInfoDto> doGetAreaClaimDataForCharts(PageRequest request) throws Exception{
		return pubDtInfoMapper.selectClaimInfoForArea(request.getParams());
	}
