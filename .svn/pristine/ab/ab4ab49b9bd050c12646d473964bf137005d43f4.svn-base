...
<div>
    <div class="">
        <form id="favorableForm">
            <input type="hidden" id="uid" value="${favorAbleInfo.uid}"/>
            <input type="hidden" id="priPID" value="${favorAbleInfo.priPID}"/>
	        ...
	        <c:if test="${!empty hisModSize && hisModSize <= 1}">
    ...
    <c:if test="${!empty hisModSize && hisModSize > 1}">
       <div id="otherPunishHis">
	    <h1 class="h4-title left mt10">历史审核记录</h1>
...
<script src="/js/reg/server/favorable/favorable_view_main.js"></script>
</body>
<script>
    window._CONFIG = {
