...
import com.icinfo.cs.yr.mapper.SfcUpdateinfoMapper;
...
/**
 * 描述:    cs_sfc_updateinfo 对应的Service接口实现类.<br>
 *
 * @author framework generator
 * @date 2016年10月10日
 */
@Service
public class SfcUpdateinfoServiceImpl extends MyBatisServiceSupport implements ISfcUpdateinfoService {
	
	@Autowired
	SfcUpdateinfoMapper sfcUpdateinfoMapper;

	@Override
	public List<SfcUpdateinfoDto> queryPage(PageRequest request) {
		PageHelper.startPage(request.getPageNum(), request.getLength());
		return sfcUpdateinfoMapper.queryPageResult(request.getParams());
	}
					...
					for (String s : strings) {
						...
						if (field1.getName() == s && field2.getName()==s) {
							...
							if((field1.get(oldEntity) != null && !"".equals(field1.get(oldEntity))) || (field2.get(newEntity) != null && !"".equals(field2.get(newEntity))) ){
								SfcUpdateinfo sfcUpdateinfo = new SfcUpdateinfo(); 
								//历史的为空，当前属性有值，添加
								if(field1.get(oldEntity)==null){
									sfcUpdateinfo.setAltBe(null);
									sfcUpdateinfo.setAltAf(String.valueOf(field2.get(newEntity)).replace("'","\\\'"));
								}
								//当前为空，历史有值  ，删除
								if(field2.get(newEntity)==null){
									sfcUpdateinfo.setAltBe(String.valueOf(field1.get(oldEntity)).replace("'","\\\'"));
									sfcUpdateinfo.setAltAf(null);
								}
								//当前的都有值且不相等，修改
								...
								sfcUpdateinfo.setAltItem(field1.getName());
								sfcUpdateinfo.setAltDate(new Date());
								sfcUpdateinfo.setAnCheID(anCheID);
								// 获取类的名字
								...
								if(tempClassName.indexOf("SFCBASEINFO")>-1){
									altID = "anCheID";	 
								}
								if(tempClassName.indexOf("SFCWEBSITEINFO")>-1){
									altID = "WEBID";	 
								}
								if(tempClassName.indexOf("SFCLICENCEINFO")>-1){
									altID = "licID";	 
								}
								if(tempClassName.indexOf("SFCBRANCHINFO")>-1){
									altID = "brId";	 
								}
								tempClassName = null;
								if(!"".equals(altID)){
									...
									sfcUpdateinfo.setAltID(id);
								}
								// 比较类名
								sfcUpdateinfo.setAltTable(className);
								sfcUpdateinfo.setCreateTime(new Date());
								sfcUpdateinfoMapper.insert(sfcUpdateinfo);
							}
						}
					}
				}
	...
	public int updateHis(String anCheID, String reportNo) throws Exception {
		SfcUpdateinfo sfcUpdateinfo = new SfcUpdateinfo();
		sfcUpdateinfo.setReportNo(reportNo);
		sfcUpdateinfo.setCreateTime(DateUtil.getSysDate());//时间戳
		Example example = new Example(SfcUpdateinfo.class);
		example.createCriteria().andEqualTo(Constants.CS_ANCHEID, anCheID).andIsNull("reportNo");
		return sfcUpdateinfoMapper.updateByExampleSelective(sfcUpdateinfo,example);
	}
	...
	public List<SfcUpdateinfoDto> queryPageForbidWord(PageRequest request) throws Exception{
		...
		if(request != null && request.getParams() !=null){
			return sfcUpdateinfoMapper.selectInfoForbidWord(request.getParams());
		}
		return new ArrayList<SfcUpdateinfoDto>();
	}
