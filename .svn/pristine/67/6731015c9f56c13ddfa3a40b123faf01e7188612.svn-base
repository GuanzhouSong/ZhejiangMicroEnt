	                    ...
	                    http.httpRequest({
	                        ...
	                        success: function (json) {
	                        	if(json.status=="success"){
	        						layer.msg("新增成功", {time: 1000}, function() {window.history.go(-1);});
	        					}else{
	        						$("#save").attr("disabled",false);
	    ...
	    function bind() {
	        util.bindEvents([ {
	            ...
	            handler: function() {
	            	...
	            	$("#copytitle").html("部门协同事务提请单（无证无照信息）");	
	            	$("#copyType").val("无证无照");
	            	$("#copyCode").val("wzwz");
	            	$("#illegalItem").val("");
	            	$("#chose_item").show();
	            ...
	            handler: function() {
	            	$(".tab-selected").removeClass("tab-selected");
	            	$(this).parent().parent().addClass("tab-selected");
	            	$("#copytitle").html("部门协同事务提请单（企业违法案件线索）");
	            	$("#copyType").val("企业违法案件");
	            	$("#copyCode").val("qywfaj");
	            	$("#illegalItem").val("");
	            	$("#chose_item").show();
	            ...
	            handler: function() {
	            	$(".tab-selected").removeClass("tab-selected");
	            	$(this).parent().parent().addClass("tab-selected");
	            	$("#copytitle").html("部门协同事务提请单（其他情况协同事务提请）");
	            	$("#copyType").val("其他情况协同事务提请");
	            	$("#copyCode").val("qt");
	            	$("#illegalItem").val("");
	            	$("#chose_item").hide();
