    ...
    function initmentDataTable() {// 初始化table
       /* var tplact = $('#tplact').html();
        ...
        table1 = dataTable.load({
            ...
            bPaginate: false,
            ajax: {
                url: '/reg/server/iminfo/actListJSON',
                data: {
                    "invId": invID
                },
...

            {
                ...
                render: function(data, type, row, meta) {
                    var del = "";
                    if (row.pubFlag != "1") {
                        del = "<a  href='#' style='display:none' class='js-deletAct' id=" + row.paidID + " title=" + row.pubFlag + ">删除</a>";
                    }

                    return " <a   href='#' style='display:none' class='js-editAct' id=" + row.paidID + " title=" + row.pubFlag + ">修改&nbsp;&nbsp;</a>" + del;
    ...
    function initDataTable() {
     /*   var tpl = $('#tpl').html();
        ...
        table = dataTable.load({
			 ...
			 */
            ajax: {
                url: '/reg/server/iminfo/prvLisJSON',
                data: {
                    "invId": invID
                },
...

            {
               ...
               /*     var context;
                    ...
                    return template(context);*/
                	 ...
                	 var del = "";
                     if (row.pubFlag != "1") {
                         del = "<a  href='#' style='display:none' id="+row.subID+" class='js-delete'   title=" + row.pubFlag + ">删除</a>";
                     }

                     return " <a   href='#' style='display:none' class='js-edit' id=" + row.subID + " title=" + row.pubFlag + ">修改&nbsp;&nbsp;</a>" + del;
