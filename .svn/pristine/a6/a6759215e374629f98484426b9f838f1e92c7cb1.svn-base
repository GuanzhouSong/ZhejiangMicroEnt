require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'jquery','jquery.serialize','laydate','jquery.multiselect'], function (layer, dataTable, util, http) {
    ...
    $('#js-add-ui').on('click',function(){
        ...
        if (batch_arr.length > 0 && batch_arr.length == 1) {
            ...
            var uniscid = $this.data('uniscid');
            var entName = $this.data('entname');
            var uniCode;
            if(regNo){uniCode = regNo}else{uniCode = uniscid}
            layer.dialog({
                title: '提请数据信息修正',
                area: ['900px', '600px'],
                content: '/reg/server/datamodify/mid_data_edit?pripid='+pripid+"&uniCode="+uniCode+"&entName="+entName,
