require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate'], function (layer, dataTable, util, http, handlebars) {
    ...
    function valifyParams(){
    	...
    	if(operationType == "audit"){
    		...
    		if(auditState == "undefined" || auditState == null || auditState == "" || auditState == undefined){
    			layer.tips("请选择审核结果",$("#auditState1"),{tips:3, tipsMore:true, ltype:0});
    			return false;
    		}
    	}
    	var auditOpinion = $("#auditOpinion").val();
    	if(auditOpinion == ""){
    		layer.tips("请输入审核意见",$("#auditOpinion"),{tips:3, tipsMore:true, ltype:0});
			return false;
    	}
    ...
    function getCommitParam(){
    	...
    	var operationType = $("#operationType").val();
    	pubScentResultDto.operationType = operationType;
