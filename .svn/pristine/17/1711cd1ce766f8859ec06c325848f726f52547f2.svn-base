require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars','jquery.form', 'jquery','jquery.serialize','laydate','jquery.multiselect'], function (layer, dataTable, util, http,handlebars) {
    ...
    function initDataTable() {
        ...
        table = dataTable.load({
            ...
            ajax: {
            	type : "POST",
                url:'/reg/server/party/member/partyadd.json',
                data:function(d){
            ...
            columnDefs : [ 
							{
								...
								render : function(data, type, row, meta) {
									if(row.auditState == '0' || row.auditState == null || row.auditState == ''){
										return "<a class='commit edit'>修改</a>&nbsp;<a class='commit delete'>删除</a>&nbsp<a class='commit create'>入库</a>";
    ...
    function bind() {
        util.bindEvents([{
            ...
            event: 'click',
            handler: function () {
            	window.location.href = '/reg/server/party/member/addChoose';
            }
        },{
            el: '.edit',
            event: 'click',
            handler: function () {
                ...
                layer.dialog({
                    title: '修改',
                    area: ['100%', '100%'],
                    content: '/reg/server/party/member/addOrModifyView?priPID='+data.priPID+"&memberUid="+data.uid+"&viewFlag=2"+"&businessType=2",
            ...
            handler: function () {
                ...
                layer.dialog({
                	title: '详情',
                	area: ['100%', '100%'],
                    content: '/reg/server/party/member/addOrModifyView?priPID='+data.priPID+"&memberUid="+data.uid+"&viewFlag=3"+"&businessType=2",
                    callback: function (data) {
                        if (data.reload) {
                        }
                    }
            ...
            handler: function () {
            	 var data = table.row($(this).closest('td')).data();
	        	 layer.confirm('请确认录入的党员信息是否入库？入库后即为建档信息只能修改不能删除。', {icon: 2, title: '提示'}, function (index) {
	        		 http.httpRequest({
                         url:'/reg/server/party/member/saveOrEdit',
                         data:{UID : data.uid,auditState:'1'},
                         serializable: false,
                         type: 'post',
                         success: function (data) {
                             if(data.status=='success'){
