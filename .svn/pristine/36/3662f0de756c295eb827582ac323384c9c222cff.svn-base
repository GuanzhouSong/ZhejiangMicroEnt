require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate'], function (layer, dataTable, util, http,handlebars) {
    ...
    function doChange(){ 
        ...
        if(pubdtinfostop=="1"){
        	 ...
        	 $("#pubdtinforemark").val("不涉及该项后置审批，终止推送");
        }else{
        	 $("#pubdtinforemark").val("");
        ...
        },
        {
            ...
            handler: function () { 
            	...
            	if((exaCode==null||exaCode=="")||(licName==null||licName=="")){
            		layer.msg("请先选择审批事项", {ltype: 0,time:2000});
    	    		return false;
            	}
                layer.dialog({
                    title: '选择审批部门',
            ...
            handler: function () { 
        	    ...
        	    if(pubdtinfostop=="1"){
        	    	...
        	    	if((deptname==null||deptname=="")||(checkDep==null||checkDep=="") ){ 
        	    		layer.msg("请选择审批机关", {ltype: 0,time:2000});
        	    		return false;
        	    	}
        	    	if(checkDep.length!=13){
        	    		layer.msg("请选择具体的审批机关", {ltype: 0,time:2000});
        	    		return false;
        	    	}
        	    }
        	    ...
        	    if(pubdtinfostop=="1"){
        	    	layer.confirm('确定要终止吗?', {icon: 3, title: '提示'}, function () { 
    	                 http.httpRequest({
    	                    url: _sysUrl+'/pubdtinfo/commitPubDtInfoHandPush',
    	                    serializable: true,
    	                    data: formParam,
    	                    type: 'post',
	    	                    success: function (data) { 
        	    	...
        	    	layer.confirm('确定要推送吗?', {icon: 3, title: '提示'}, function () {
        	    		var pushFlag=false; 
                	    http.httpRequest({
                            url: _sysUrl+'/pubdtinfo/checkPush',
                            serializable: true,
                            data: formParam,
                            async:false,
                            type: 'post',
        	                    success: function (data) {  
        	                    	if (data.status == 'success'&& (data.data=="true"||data.data==true)) { 
        	                    		pushFlag=true;
                                        layer.msg("此企业同一审批事项该审批部门已经推送，不能重复推送", {time: 2000}, function () {
                                         });
                                }
                            }
                        }); 
