        ...
        util.bindEvents([{
            ...
            handler: function () {
            	...
            	var formParam=$('#params').serializeObject();
            	 http.httpRequest({
                     url: '/reg/server/registmanage/doPrintLiq',
                     serializable: false,
                     data:formParam,
                     type: 'post',
                     success: function (data) { 
                    	 if(data.status=="success"){
                    		 ...
                    		 layer.msg("打印清算通知书保存成功", {time: 1000}, function () {
		   			       	     var _id = "_doPrint_"+new Date().getTime();
		   			             var url = "/reg/server/registmanage/printLiqView?uid="+uid+"&removeUid="+removeUid;
		   			             try{
