...
import com.icinfo.cs.rpt.rptmapper.RptSmImportMapper;
...
/**
 * 描述:    cs_rpt_sm_import 对应的Service接口实现类.<br>
 *
 * @author framework generator
 * @date 2017年05月05日
 */
@Service
public class RptSmImportServiceImpl extends MyBatisServiceSupport implements IRptSmImportService {
	
	private static final Logger logger = LoggerFactory.getLogger(RptSmImportServiceImpl.class);

	@Autowired
	private  RptSmImportMapper rptSmImportMapper;
	...
	public List<RptSmImportDto> selectRptSmImportCount(PageRequest request) throws Exception {
		try {
			...
			return rptSmImportMapper.selectRptSmImportCount(searchMap);
		} catch (Exception e) {
			e.printStackTrace();
