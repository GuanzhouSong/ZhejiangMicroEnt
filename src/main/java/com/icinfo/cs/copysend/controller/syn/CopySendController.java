...
public class CopySendController extends CSBaseController {
	@Autowired
	 ...
	 * @throws Exception
	 */
	public void opearateRedis() throws Exception{
		//操作成功后  缓存重新设置
		Map<String, Object> forBidmap = new HashMap<String, Object>();
		SysUserDto sysUserDto = (SysUserDto)getSession().getAttribute("session_sys_user");
		String regOrg = selectRegorgByDeptCode(sysUserDto);
		creatOptDBAuthEnv(forBidmap, "t.CheckDep", "t.LocalAdm");
		forBidmap.put("regOrg", regOrg);
	    forBidmap.put("userType", sysUserDto.getUserType());
		if("1".equals(sysUserDto.getUserType())){//警示
				forBidmap.put("departCode", sysUserDto.getDeptCode());
		}else{//协同
				forBidmap.put("departCode", sysUserDto.getSysDepart().getAdcode());
				forBidmap.put("dutyDeptCodes", sysUserDto.getDutyDeptCodes());
		}
		//查询当前待审核数量
	    int coprSendCount = copySendMapper.selectDaiShenHeNum(forBidmap);
		regIndexService.dodoRedisRegcountMod(sysUserDto.getId(), forBidmap, 13, coprSendCount,sysUserDto);
	}
