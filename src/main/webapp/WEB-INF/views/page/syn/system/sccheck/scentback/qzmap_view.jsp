    ...
    <div class="main">
        <div class="actual-wrap actual-w clearfix">
            ...
            <div class="map-box map-quzhou fl">
                ...
                <div class="city city-0"><i></i>衢州市本级<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 1}">
                <div class="city city-1">开化县<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 2}">
                <div class="city city-2">常山县 <span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 3}">
                <div class="city city-3">江山市<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 4}">
                <div class="city city-4">柯城区<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 5}">
                <div class="city city-5">龙游县<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 6}">
                <div class="city city-6">衢江区<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                </c:forEach>
            </div>
