        ...
        <div class="actual-wrap clearfix">
            ...
            <div class="map-box map-jh fl">
             <c:forEach var="pubScentDto" items="${pubScentDtoList}" varStatus="status">
                ...
                <div class="city city-0"><i></i>金华市本级<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 1}">
                <div class="city city-1">金东区<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 2}">
                <div class="city city-2">浦江县<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 3}">
                <div class="city city-3">义乌市<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 4}">
                <div class="city city-4">东阳市<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 5}">
                <div class="city city-5">磐安县<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  ==6}">
                <div class="city city-6">永康市<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 7}">
                <div class="city city-7">武义县<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 8}">
                <div class="city city-8">婺城区<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 9}">
                <div class="city city-9">开发区<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 10}">
                <div class="city city-10">金华山<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                <c:if test="${status.index  == 11}">
                <div class="city city-11">兰溪市<span>${pubScentDto.specialNum }</span></div>
                </c:if>
                </c:forEach>
            </div>
