            ...
            <div class="map-box map-huz fl">
            	<c:forEach var="pubScentDto" items="${pubScentDtoList}" varStatus="status">
            		<c:if test="${status.index  == 0}">
		                <div class="city city-0"><i></i>湖州市本级<span>${pubScentDto.specialNum }</span></div>
            		</c:if>
            		<c:if test="${status.index  == 1}">
		                <div class="city city-1">长兴县<span>${pubScentDto.specialNum }</span></div>
            		</c:if>
            		<c:if test="${status.index  == 2}">
	                	<div class="city city-2">度假区<span>${pubScentDto.specialNum }</span></div>
	                </c:if>
	                <c:if test="${status.index  == 3}">
	                	<div class="city city-3">吴兴区<span>${pubScentDto.specialNum }</span></div>
	                </c:if>
	                <c:if test="${status.index  == 4}">
	                	<div class="city city-4">南浔区<span>${pubScentDto.specialNum }</span></div>
	                </c:if>
	                <c:if test="${status.index  == 5}">
	                	<div class="city city-5">开发区<span>${pubScentDto.specialNum }</span></div>
	                </c:if>
	                <c:if test="${status.index  == 6}">
	                	<div class="city city-6">德清县<span>${pubScentDto.specialNum }</span></div>
	                </c:if>
	                <c:if test="${status.index  == 7}">
	                	<div class="city city-7">安吉县<span>${pubScentDto.specialNum }</span></div>
	                </c:if>
            	</c:forEach>
            </div>
