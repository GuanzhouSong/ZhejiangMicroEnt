            ...
            <table class="table-public table-contact" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr>
                    <td class="odd">
                        ...
                        <div class="item-txt">${pbBaseinfoHis.traName}</div>
                    </td>
                    <td class="odd">
                        <div class="item-name">经营者姓名：</div>
                        <div class="item-txt">${pbBaseinfoHis.name}</div>
                    </td>
                </tr>
                <tr>
                    <td class="odd">
                        <div class="item-name">统一社会信用代码/注册号：</div>
                        <div class="item-txt">${empty pbBaseinfoHis.uniCode ? pbBaseinfoHis.regNO : pbBaseinfoHis.uniCode}</div>
                    </td>
                    <td class="odd">
                        <div class="item-name">联系电话：</div>
                        <div class="item-txt">${pbBaseinfoHis.tel}</div>
                    </td>
                </tr>
                <tr>
                    <td class="odd">
                        <div class="item-name">资金数额：</div>
                        <div class="item-txt"><fmt:formatNumber value="${pbBaseinfoHis.fundAm}" pattern="###,###,###,##0.0000"/>万元</div>
                    </td>
                    <td class="odd">
                        <div class="item-name">从业人数：</div>
                        <div class="item-txt">${pbBaseinfoHis.empNum}人</div>
                    </td>
                </tr>
                </tbody>
            </table>
        ...
        <div class="content content-contact">
          <div >
                <table class="table-public table-web" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tbody >
                    	<c:forEach var="pbLicenceinfoHis" items="${pbLicenceinfoHisList}" varStatus="status">
			        		<tr>
			        			<td >${pbLicenceinfoHis.licNameCN}</td> 
			        			<td >
			        			<fmt:formatDate var="pbvalTo"
								value="${pbLicenceinfoHis.valTo}"
								pattern="yyyy-MM-dd" />
        ...
        <div class="content content-contact">
            <div>
                ...
                <table class="table-public table-web" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tbody >
                    	<c:forEach var="pbWebsiteinfoHis" items="${pbWebsiteinfoHisList}" varStatus="status">
			        		<tr>
			        			<td >
			        			 <c:if test="${pbWebsiteinfoHis.webType==1}">网站</c:if>
			        			 <c:if test="${pbWebsiteinfoHis.webType==2}">网店</c:if> 
			        			</td>
			        			<td > ${pbWebsiteinfoHis.webSitName}</td>
			        			<td >${pbWebsiteinfoHis.webSite} </td>
			        		</tr>
		        		</c:forEach> 
                    </tbody>
                </table>
            ...
            <table class="table-public table-contact" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr>
                    <td class="odd">
                        ...
                        <div class="item-txt">
                        <c:if test="${pbBaseinfoHis.vendIncIsPub == '1'}"><fmt:formatNumber value="${pbBaseinfoHis.vendInc}" pattern="###,###,###,##0.0000"/>万元</c:if>
                    	<c:if test="${pbBaseinfoHis.vendIncIsPub == '0'}">个体工商户选择不公示</c:if></div>
                    </td>
                    <td class="odd">
                        <div class="item-name">纳税总额：</div>
                        <div class="item-txt">
                        <c:if test="${pbBaseinfoHis.vendIncIsPub == '1'}"><fmt:formatNumber value="${pbBaseinfoHis.ratGro}" pattern="###,###,###,##0.0000"/>万元</c:if>
                        <c:if test="${pbBaseinfoHis.vendIncIsPub == '0'}">个体工商户选择不公示</c:if>
                        </div>
                    </td>
                </tr>
                </tbody>
