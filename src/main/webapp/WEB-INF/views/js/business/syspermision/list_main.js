require(['common/util', 'component/iframeLayer', 'common/http', 'handlebars', 'layer1', 'ztree'], function (util, layer, http, handlebars) {
    ...
    function showPermision(id, parentNode) {
        ...
        http.httpRequest({
            type: 'post',
            url: '/reg/server/syspermision/query',
            data: {id: id},
            success: function (data) {
                if (data.status == 'success') {
        ...
        util.bindEvents([{
            ...
            handler: function () {
                ...
                layer.dialog({
                    title: '新增权限',
                    area: ['50%', '50%'],
                    content: '/reg/server/syspermision/show?sid=' + sid,
            ...
            handler: function () {
                ...
                layer.dialog({
                    title: '修改权限',
                    area: ['50%', '50%'],
                    content: '/reg/server/syspermision/show?id=' + id,
                    callback: function (data) {
                        //重新加载列表数据
                        if (data.reload) {
                            ztreeInit();
            ...
            handler: function () {
                ...
                layer.confirm('确定要删除该权限资源吗?', {icon: 2, title: '提示'}, function (index) {
                    http.httpRequest({
                        type: 'post',
                        url: '/reg/server/syspermision/delete',
                        data: {id: id},
                        success: function (data) {
                            if (data.status == 'success') {
                                if (data.msg) {
            ...
            handler:function () {
                http.httpRequest({
                    type:"GET",
                    url: '/reg/server/syspermision/reload',
                    success:function (data) {
                        var errorMsg = data.msg;
                        var errors = data.errors;
                        if (errors.length > 0) {
