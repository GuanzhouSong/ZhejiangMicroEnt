require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars','jquery.form', 'jquery','jquery.serialize','laydate','jquery.multiselect','jquery.validate','common/validateRules',], function (layer, dataTable, util, http,handlebars) {
    ...
    function init() {
        ...
        var auditState = window._CONFIG.auditState;
    ...
    function bind() {
        util.bindEvents([{
            ...
            handler: function () {
            	...
            	$("#topInfo").hide();
                http.httpRequest({
                	 url: '/reg/server/party/member/getDefaultPartyOrg',
                     serializable: false,
                     data: {priPID:$("#priPID").val()},
                     type: 'get',
                     success: function (data) {
                   	 if(data.status == 'success') {
                   		 console.log(data);
                   		 $("#partyOrgID").val(data.data.uid);
                   		 $("#partyOrgName").val(data.data.partyOrgName).attr("readonly",true);
                   	 }
                   }
                })
             }
