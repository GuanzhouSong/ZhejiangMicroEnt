require(['component/iframeLayer', 'common/util', 'common/http', 'jquery.validate', 'jquery.serialize','common/validateRules','laydate'], function (layer, util, http) {
    ...
    function bind() {
        util.bindEvents([{
            ...
            event: 'change',
            handler: function () {
            	$("#auditRemarkTem").val("同意责令整改");
            }
        },{
            el: '#auditStateTemN',
            event: 'change',
            handler: function () {
            	$("#auditRemarkTem").val("退回修改");
            }
        },{
            el: '#auditStateTemC',
            event: 'change',
            handler: function () {
            	$("#auditRemarkTem").val("取消责令整改");
            }
        },{
            ...
            handler: function () {
               ...
               var url = "/reg/server/orderreformset/orderreFormsetApplyPrint?uid="+$("#uid").val()+"&viewFlag="+_viewFlag;
               try{
            ...
            handler: function () { 
            	...
            	if(_viewFlag=="5"){ 
            	 ...
            	 http.httpRequest({
                     url: '/reg/server/orderreformset/commitAuditOrderReformSet',
                     serializable: true,
                     data:formParam,
                     type: 'post',
