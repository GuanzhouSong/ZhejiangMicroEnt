require(['component/iframeLayer', 'common/util',  'common/http', 'jquery.validate','jquery.serialize', 'jquery.placeholder','jquery.icinfo'], function (layer, util, http) {
    ...
    function bind() {
        	...
        	
        {
         ...
         handler:function(){
        	 ...
        	 $("#changeForm").append('<input type="hidden" name="subID" value='+licID+'>');
        	 $("#changeForm").append('<input type="hidden" name="invID" value='+invID+'>');
        	 $("#changeForm").append('<input type="hidden" name="conFormCN" value='+conFormCN+'>');
        	 ...
        	 if(change!=null&&change!=""&&change>999999999999){
        		 layer.tips("请输入0.0001-99999999999的数值",$("#change2"),{tips:3,tipsMore:true,ltype:0});
        	     return false;
        	 }
        ...
        $("#savechange").attr("disabled", true);
   		 $("#savechange").val("提交中...."); 
        	 var _index;
   http.httpRequest({
                 url: '/reg/client/im/ent/inprodetail/change',
                 beforeSend:function(){
