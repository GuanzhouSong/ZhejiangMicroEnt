require(['common/util', 'layer1', 'common/http', 'jquery.validate', 'jquery','jquery.serialize','jquery.placeholder','jquery.icinfo'], function (util, layer, http) {
    ...
    function bind() {
        util.bindEvents([{
            ...
            event: 'click',
            handler: function () {
            	var url = window._CONFIG.casServer + window._CONFIG.webServer + "/reg/client/login/cas/3";
            ...
            handler: function () {
            	...
            	if(_regno == "" || _regno.length <= 6) {
	            	...
	            	http.httpRequest({
	                    ...
	                    data: {regno : $.trim($("#regno").val()), type : '3'},
...
//	                                }
            ...
            handler: function () {
                ...
                http.httpRequest({
                    ...
                    success: function (data) {
                    	if (data.status == 'success') {
                            location.assign('/reg/client/yr/yrregcheck/sfc/index_list');
                            return;
                        }else {
                        	$("#errorinfo").html(data.msg);
                        	$(".fr .fr-tab").hide();
            ...
            handler: function () {
				//确认联络员登录的跳转至联络员登录填报页
                var url = "/reg/client/login/phoneapply/list?entage=2";
                window.location.href = url; 
            }
        },{
            ...
            handler: function () {
            	...
            	if(_entRegNo != "" && _entRegNo.length > 6){  //注册号或统一代码不为空
            		...
            		http.httpRequest({
            		   ...
            		   data: {pasentkey :$("#entRegNo").val(),encatge : 2},
            		   success: function(data){
            			   $(".fr .fr-tab").hide();
            			   if(data.status == 'success'){
...
//         				   } 
         				//确认联络员登录的跳转至联络员登录填报页
     	                var url = "/reg/client/login/phoneapply/list?regNo=" +regNo+ "&entage=2";
     	                window.location.href = url;
            			   }else{
