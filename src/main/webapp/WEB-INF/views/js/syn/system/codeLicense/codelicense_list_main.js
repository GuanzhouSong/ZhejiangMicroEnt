        ...
        util.bindEvents([{
            ...
            handler: function () {
            	 layer.dialog({
                     title: '新增',
                     area: ['900px', '400px'],
                     content: '/common/system/codelicense/codeLicenseAddPage?exaCode=null',
            ...
            handler: function () { 
            	var data = table.row($(this).closest('td')).data();
                 ...
                 layer.dialog({
                    title: '修改',
                    area: ['60%', '50%'],
                    content: '/common/system/codelicense/codeLicenseViewPage?exaCode='+data.exaCode+'&viewFlag=1'+'&licType='+data.licType,
                    callback: function (data) {
        ...
        },
        {
            ...
            handler: function () { 
            	var data = table.row($(this).closest('td')).data();
                 layer.dialog({
                    title: '查看',
                    area: ['60%', '50%'],
                    content: '/common/system/codelicense/codeLicenseViewPage?exaCode='+data.exaCode+'&viewFlag=2'+'&licType='+data.licType,
                    callback: function (data) {
                    	 if (data.reload) { //close是否有传递此参数
                             table.ajax.reload();
                         }
            ...
            handler: function () { 
            	...
            	layer.confirm('后置审批事项需和准入系统保持一致，若非准入系统的后置审批事项有变化，否则请勿执行删除操作,确定要删除吗?', {icon: 2, title: '提示'}, function () { 
	                http.httpRequest({
	                    type: 'post',
	                    url: '/common/system/codelicense/deleteCodeLicense',
