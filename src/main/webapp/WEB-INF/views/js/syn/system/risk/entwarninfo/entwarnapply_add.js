require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate'], function (layer, dataTable, util, http,handlebars) {
	...
	function initDataTable() {
		...
		columns : [
			...
			{data : 'entTypeName',width:'130px', className: 'center'}, 
			{data : 'industryCoName',width:'130px', className: 'center'}, 
			...
			{data : null, width:'130px',className: 'center'}
		],
		columnDefs : [
       {
	   targets: 0,
          	 ...
          	 if($(this).is(':checked')){
          		...
          		if(flag){
          	        ...
          	        $('#enttbody').append("<tr><td><a href='javascript:void(0)' class='remove' id='"+data.priPID+"'>删除</td><td>"+(data.uniscid==null?"":data.uniscid)+"</td><td>"+data.regNO+"</td><td>"
          	   	    +data.entName+"</td><td>"+(data.leRep == null?"":data.leRep)+"</td><td>"+data.entTypeName+"</td><td>"
          	     	+(data.industryCoName == null?"":data.industryCoName)+"</td><td>"+(data.dom==null?"":data.dom)+"</td><td>"+data.regOrg+"</td><td>"+(data.localAdmName==null?"":data.localAdmName)+"</td><td>"+(data.isRiskWarn=='Y'?"<a class='mr10 lookhis' id='"+data.priPID+"'>有</a>":"无")+"</td></tr>");	  
          		 }
      		 }
   		 });
      	 $('#entNum').text(entNum);
        ...
        util.bindEvents([
            ...
            handler: function () {
            	...
            	layer.dialog({
                    title: '添加企业标识',
                    area: ['80%', '80%'],
                    content: '/syn/risk/csentwarn/enaddmark?arr='+arr,
