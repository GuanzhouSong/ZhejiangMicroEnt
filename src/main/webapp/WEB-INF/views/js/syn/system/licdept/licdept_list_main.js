   ...
   // laydate.skin('molv');//初始化皮肤 (墨绿)
    ...
    function initDataTable() {
        table = dataTable.load({
            ...
            showIndex: true,
            ajax: {
                url:_sysUrl+'/licdept/licDeptListJSON.json',
                data:function(d){
             ...
             columnDefs : [
							{
							    ...
							    render: function (data, type, row, meta) {
							    	return '<a href="javascript:void(0);" class="moddifty">修改</a>'+
    ...
    function bind() {
        util.bindEvents([{
            ...
            handler: function () {
            	 layer.dialog({
                     title: '新增',
                     area: ['65%', '70%'],
                     content: _sysUrl+'/licdept/licdeptEditPage?flag=1',
        ...
        },
        {
            ...
            handler: function () { 
            	 layer.dialog({
                     title: '数据下放',
                     area: ['50%', '40%'],
                     content: _sysUrl+'/licdept/transferExaCodePage',
                     callback: function (data) { 
        ...
        },
        {
            ...
            handler: function () { 
                 ...
                 layer.dialog({
                    title: '修改',
                    area: ['65%', '70%'],
                    content: _sysUrl+'/licdept/licdeptEditPage?uid='+uid+"&flag=2",
                    callback: function (data) {
                    	 if (data.reload) { //close是否有传递此参数
                             table.ajax.reload();
                         }
            ...
            handler: function () { 
            	...
            	var uid=data.uid;
                 layer.dialog({
                    title: '查看',
                    area: ['65%', '70%'],
                    content: _sysUrl+'/licdept/licdeptEditPage?uid='+uid+"&flag=3",
                    callback: function (data) {
                    }
                })
            }
        },
        {
            ...
            handler: function () {
            	 http.httpRequest({
                     url: _sysUrl+'/licdept/executeLicDeptDataByLicZone',
                     serializable: false,
                     data: null,
                     type: 'post',
                     success: function (data) { 
