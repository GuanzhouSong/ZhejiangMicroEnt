require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars','jquery.form', 'jquery','jquery.serialize','laydate','jquery.multiselect'], function (layer, dataTable, util, http,handlebars) {
    ...
    function initDataTable() {
        ...
        table = dataTable.load({
            ...
            ajax: {
                url:'/syn/server/sccheck/codemarktype/list.json',
                data:function(d){
            ...
            columns: [
                {data: '_idx'},
                {data: null},
                {data: 'markName',cut: {length: 20,mark:'...'}},
                {data: 'specialName',cut: {length: 20,mark:'...'}},
        ...
        util.bindEvents([{
            ...
            handler: function () {
                layer.dialog({
                    title: '添加',
                    area: ['60%', '80%'],
                    content: '/syn/server/sccheck/codemarktype/edit?uid=',
            ...
            handler: function () {
                ...
                layer.dialog({
                    title: '修改',
                    area: ['60%', '80%'],
                    content: '/syn/server/sccheck/codemarktype/edit?uid='+data.uid+'&viewFlag=1',
                    callback: function (data) {
                        if (data.reload) {
                        	table.ajax.reload();
                        }
            ...
            handler: function () {
                ...
                layer.dialog({
                	title: '查看',
                	area: ['60%', '80%'],
                    content: '/syn/server/sccheck/codemarktype/edit?uid='+data.uid+'&viewFlag=2',
                    callback: function (data) {
                        if (data.reload) {
                        }
                    }
