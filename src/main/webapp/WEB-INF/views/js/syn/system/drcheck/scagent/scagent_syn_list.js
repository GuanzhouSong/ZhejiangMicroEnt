            ...
            columnDefs: [
			 {
			    ...
			    render: function (data, type, row, meta) {
			    	...
			    	if(userUid == '21086d395cd04e4981e9afdc8f060a22'){//黄总账号特批删除权限
			    		return "<a class='commit js_update'>修改</a><a class='commit js_show'>详情</a><a class='commit js_del'>删除</a>";
			    	}else{
				...
				targets: 16,
			    render: function (data, type, row, meta) {
			    	if(row.slicenNOName !='' && row.slicenNOName !=null){
			    		return row.slicenNOName;
        ...
        util.bindEvents([
				{
				    ...
				    handler: function () {
				    	...
				    	layer.dialog({
				            title: '详情',
				            area: ['90%', '90%'],
				            content:'/syn/server/drcheck/pubscagent/show?uid='+uid+'&detailsFlag=1',
				            callback: function (data) {
				                if (data.reload) { 
				                	table.ajax.reload();
				                }
				    ...
				    handler: function () {
				    	var data = table.row($(this).closest('td')).data();
				    	layer.confirm('您确认要删除该执法人员？', {icon: 2, title: '提示'}, function (index) {
				    		http.httpRequest({
		                        url: '/syn/server/drcheck/pubscagent/delete',
		                        serializable: false,
		                        data: {uid : data.uid},
		                        type: 'post',
              ...
              handler: function () {
                  ...
                  layer.dialog({
                      ...
                      callback: function (data) {
                          if(data.adCodes){
                        	  if(data.adCodes.length >= 8){
                        		  $("#deptCodeArr").val(data.adCodes.substr(0,8));
                        	  }
